...
void GptJ<T>::allocateBuffer(size_t batch_size, size_t beam_width, size_t max_seq_len, size_t max_input_len)
{
    ...
    if (cudaError != 0) {
	    std::cerr << "Error occured when invoking cudaHostAlloc: " << cudaError << std::endl;
    }
    cudaError = cudaHostGetDevicePointer(&finished_buf_, h_finished_buf_, cudaHostAllocMapped);
    if (cudaError != 0) {
	    std::cerr << "Error occured when invoking cudaGetDevicePointer: " << cudaError << std::endl;
    }
...
void GptJ<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        ...
        cudaError_t cudaError = cudaHostFree(h_finished_buf_);
	if (cudaError != 0) {
		cerr << "Error occured when invoking cudaHostFree" << cudaError << std::endl;
	}
